...
function observationsByPrivacy(data, privacy) 
{
  ...
  let newObservations = [];
    if (privacy === null) 
    {
      for(const result of data.results) 
      {
        if (result.privacy === null) 
        {
          newObservations.push(result);
        }
      }
        return newObservations;
      } 
      
    else if (typeof privacy === 'string' && !['open', 'obscured'].includes(privacy.toLowerCase())) 
    {
      throw new Error();
    } 
    
    else if (typeof privacy !== 'string') 
    {
      throw new Error();
    }
      for(const result of data.results) 
      {
          if (result.privacy === privacy.toLowerCase()) 
          {
              newObservations.push(result);
          }
      }
...
function transformObservation(original) 
{
  ...
  //fix code
    const transformed = 
    {
      ...
      geoCoords: original.location.split(',').reverse().map((coord) => Number(coord)),
...
function transformObservations(data) 
{
  ...
  let newObservations = [];
    ...
    for (const result of data.results) 
    {
      newObservations.push(transformObservation(result));
    }
...
function getObservationsById(data, ...ids) {
  ...
  //fix code
    let newObservations = [];
        ids.forEach((id) => 
        {
          const foundData = data.results.find((result) => result.id === id);
      
          if (foundData !== undefined) 
          {
              newObservations.push(foundData);
          }
        }
        );
        ...
        else if (newObservations.length === 1) 
          {
...
function getObservationsByPositionalAccuracy(data, options = {}) 
{
  ...
  const optionKeys = Object.keys(options);
    ...
    if (optionKeys.length === 0 || !optionKeys.some(e => ['eq', 'gte', 'lte'].includes(e))) 
    {
      return data.results;
    } 
    
    else if (optionKeys.includes('eq')) 
    {
      return data.results.filter((result) => result.positional_accuracy === options.eq);
    } 
    else if (optionKeys.length === 2) 
    {
        return data.results.filter((result) => result.positional_accuracy >= options.gte &&
        result.positional_accuracy <= options.lte);
    }

  let newObservations = [];
      data.results.forEach((result) => 
      {
          if (options.gte && typeof options.gte === 'number' && result.positional_accuracy >= options.gte) 
          {
              newObservations.push(result);
          } 
          
          else if (options.lte && typeof options.lte === 'number' && result.positional_accuracy <=
          options.lte) 
          {
            newObservations.push(result);
          }
...
function getTaxonPhotos(data) {
  ...
  //fix code
  const newObservations = data.results.map((result) => 
  {
    if (result.taxon && result.taxon.default_photo) {
      return {
        original: `https://static.inaturalist.org/photos/${result.taxon.default_photo.id}/original.jpg?${result.id}`,
        square: `https://static.inaturalist.org/photos/${result.taxon.default_photo.id}/square.jpg?${result.id}`,
        small: `https://static.inaturalist.org/photos/${result.taxon.default_photo.id}/small.jpg?${result.id}`,
        medium: `https://static.inaturalist.org/photos/${result.taxon.default_photo.id}/medium.jpg?${result.id}`,
        large: `https://static.inaturalist.org/photos/${result.taxon.default_photo.id}/large.jpg?${result.id}`,
      };
    }
  });

  return newObservations.filter(e => e !== undefined);
}
/*******************************************************************************
...
function getUserStats(data) {
  // TODO
    ...
    const { observations_count, journal_posts_count, species_count } = result.user;
    
      if (observations_count) object.totals.observations += observations_count;
      if (journal_posts_count) object.totals.journals += journal_posts_count;
      if (species_count) object.totals.species += species_count;
      }
    });
    if (object.count > 0) 
    {
      object.averages.observations = object.totals.observations / object.count;
      object.averages.journals = object.totals.journals / object.count;
      object.averages.species = object.totals.species / object.count;
    }
...
function extractTimeZones(data) {
  ...
  const newObservations = [];
    ...
    data.results.forEach((result) => {
      ...
      const timeZone = result.created_time_zone;
        if (!newObservations.includes(timeZone)) 
        {
            newObservations.push(timeZone);
        }
      });
...
function extractTimeZones2(data) {
  ...
  //fix code
    ...
    const newObservations = new Set();
  data.results.forEach((result) => {
  
      const timeZone = result.created_time_zone;
      newObservations.add(timeZone);
    });
  
    return Array.from(newObservations);
}
