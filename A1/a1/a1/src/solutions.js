...
function createIframe(src, height, allowFullScreen) 
{
  ...
  if (typeof height === 'number') 
    
    {
      ...
      {
        return `<iframe src="${src.trim().toLowerCase()}"></iframe>`;
      }
    }
        return `<iframe src="${src.trim().toLowerCase()}" height="${height}"${
...
function fixPostalCode(postalCode) 
{
  ...
  //checking the format of postal code, if it invalid thrown an error else return the postal code.
...

{
  ...
  const shortForm = 
  {
    ...
    Y: 'YT'
      ...
      let checkEntries = Object.entries(fullForm);
        if (useShortForm) 
        {
            checkEntries = Object.entries(shortForm);
        }
          for (const [key, value] of checkEntries) 
          {
            if (key.includes(postalCode.charAt(0))) 
            {
                return value;
            }
          }
...
function countForProvince(provinceCode, ...postalCodes) 
{
    ...
    if (postalCodes.some((code) => typeof code !== 'string')) 
    {
      throw new Error();
    }
        let count = 0;
        ...
        for (let postalCode of postalCodes) 
          {
              postalCode = toProvince(postalCode, true);
...
function generateLicenseLink(licenseCode, targetBlank) 
{
    ...
    if (licenseCode) 
    {
        var normalize = licenseCode.replace('CC-', '').toLowerCase();
    }
      const f = 
      {
    ...
    if (Object.keys(f).includes(normalize)) 
    {
        return `<a href="https://creativecommons.org/licenses/${normalize}/4.0/"${
        targetBlank ? ` target="_blank"` : ''
    }>${f[normalize]}</a>`;
    }
        return `<a href="https://choosealicense.com/no-permission/"${
        targetBlank ? ` target="_blank"` : ''
...
function pureBool(value) 
{
      ...
      if (typeof value === 'number') 
      {
          ...
          const trueFormat = 'yes, y, oui, o, t, true, vrai, v'.split(', ');
          const falseFormat = 'no, non, n, f, false, faux'.split(', ');
                if (typeof value === 'string') 
                {
                  if (trueFormat.includes(value.toLowerCase())) 
                  {
                    return true;
                  } 
                  
                  else if (falseFormat.includes(value.toLowerCase())) 
                  {
                    return false;
                  }
                }
...
function buildUrl(query, order, count, license) 
{
        ...
        let queryList = [];
            ...
            if (query) 
            {
                queryList.push(`query=${encodeURIComponent(query)}`);
            } 
            ...
            if (order) 
            {
                if (['ascending', 'descending', 'asc', 'desc'].includes(order)) 
                {
                    if (order === 'asc' || order === 'ascending') 
                    {
                        queryList.push(`order=ascending`);
                    } 
                    else 
                    {
                        queryList.push(`order=descending`);
                    }
                }
            ...
            {
                queryList.push(`license=${license}`);
            } 
